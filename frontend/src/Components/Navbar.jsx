        ...
        <div ref={ref} className=" ">
            <nav className={` top-0 left-0 bg-green-950 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                <div className="hidden md:flex items-center gap-4 lg:gap-8">
                    {navLinks.map((link, i) => (
                        <NavLink key={i} to={link.path} className={`group flex flex-col gap-0.5 font-primary ${isScrolled ? "text-gray-700" : "text-white"}`}>
                            {link.name}
                ...
                <div className="hidden md:flex items-center gap-4">
                    ...
                    {
                        token ?(
                            <button onClick={logOutUser} className={`cursor-pointer px-8 py-2.5 rounded-md ml-4 transition-all duration-500 ${isScrolled ? "text-white bg-black" : "bg-red-500 text-white font-primary"}`}>
                                Logout
                            </button>
                        )
                        :
